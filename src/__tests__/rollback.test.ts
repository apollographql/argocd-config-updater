...
`;
  ...
  describe("explicit target SHA", () => {
    ...
    it("throws (no-op) when the current SHA resolves to an identical ref and tag", async () => {
      const reader = new FakeGitHistoryReader([
        { commit: "c-head", contents: valuesFile(CURRENT) },
        { commit: "c-same", contents: valuesFile(CURRENT) },
        { commit: "c-prev", contents: valuesFile(PREVIOUS) },
      ]);
    ...
    it("rolls back the tag alone when the current SHA was previously deployed with a different tag", async () => {
      const REBUILT = { ...CURRENT, prodTag: "main---0000301-rebuild" };
      const reader = new FakeGitHistoryReader([
        { commit: "c-head", contents: valuesFile(REBUILT) },
        { commit: "c-orig", contents: valuesFile(CURRENT) },
        { commit: "c-prev", contents: valuesFile(PREVIOUS) },
      ]);
    ...
    it("explains that the current SHA has no earlier deploy, rather than claiming it was never deployed", async () => {
      const reader = new FakeGitHistoryReader([
        { commit: "c-head", contents: valuesFile(CURRENT) },
        { commit: "c-prev", contents: valuesFile(PREVIOUS) },
      ]);
      ...
      ).rejects.toThrow(/is the current deploy and has no earlier deploy/);
    });
