...
const pipeline = require$$0$1.promisify(Stream.pipeline);
...
class Body {
	constructor(body, {
		...
		} else if (isBlob(body)) ; else if (require$$0.Buffer.isBuffer(body)) ; else if (require$$0$1.types.isAnyArrayBuffer(body)) {
			// Body is ArrayBuffer
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./multipart-parser-tsur63Iy.cjs'); });
...
Body.prototype.buffer = require$$0$1.deprecate(Body.prototype.buffer, 'Please use \'response.arrayBuffer()\' instead of \'response.buffer()\'', 'node-fetch#buffer');
...
Object.defineProperties(Body.prototype, {
	...
	data: {get: require$$0$1.deprecate(() => {},
...
const getNonSpecFormDataBoundary = require$$0$1.deprecate(
...
const extractContentType = (body, request) => {
	...
	if (require$$0.Buffer.isBuffer(body) || require$$0$1.types.isAnyArrayBuffer(body) || ArrayBuffer.isView(body)) {
		return null;
	}
...
class Headers extends URLSearchParams {
	...
	constructor(init) {
		...
		if (init instanceof Headers) {
		...
		} else if (init == null) ; else if (typeof init === 'object' && !require$$0$1.types.isBoxedPrimitive(init)) {
			...
			if (method == null) {
				...
				result = [...init]
					.map(pair => {
						if (
							typeof pair !== 'object' || require$$0$1.types.isBoxedPrimitive(pair)
						) {
...
const doBadDataWarn = require$$0$1.deprecate(() => {},
...
//# sourceMappingURL=index-_QvGCUZF.cjs.map
