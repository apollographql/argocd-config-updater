...
const pipeline = require$$1.promisify(Stream.pipeline);
...
class Body {
	constructor(body, {
		...
		} else if (isBlob(body)) ; else if (node_buffer.Buffer.isBuffer(body)) ; else if (require$$1.types.isAnyArrayBuffer(body)) {
			// Body is ArrayBuffer
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./multipart-parser-B35i9fHP.cjs'); });
...
Body.prototype.buffer = require$$1.deprecate(Body.prototype.buffer, 'Please use \'response.arrayBuffer()\' instead of \'response.buffer()\'', 'node-fetch#buffer');
...
const getNonSpecFormDataBoundary = require$$1.deprecate(
...
const extractContentType = (body, request) => {
	...
	if (node_buffer.Buffer.isBuffer(body) || require$$1.types.isAnyArrayBuffer(body) || ArrayBuffer.isView(body)) {
		return null;
	}
...
class Headers extends URLSearchParams {
	...
	constructor(init) {
		...
		if (init instanceof Headers) {
		...
		} else if (init == null) ; else if (typeof init === 'object' && !require$$1.types.isBoxedPrimitive(init)) {
			...
			if (method == null) {
				...
				result = [...init]
					.map(pair => {
						if (
							typeof pair !== 'object' || require$$1.types.isBoxedPrimitive(pair)
						) {
...
const doBadDataWarn = require$$1.deprecate(() => {},
...
//# sourceMappingURL=index-FuldNPo7.cjs.map
